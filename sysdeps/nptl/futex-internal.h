...
/* If *FUTEX_WORD == EXPECTED, wake up to NR_WAKE waiters on FUTEX_WORD
   and requeue up to NR_REQUEUE of the remaining waiters to TARGET_WORD.
   Returns the total number of woken and requeued waiters, or a negated
   error code (-EAGAIN if the futex word does not match EXPECTED).  */
static __always_inline int
__futex_cmp_requeue (unsigned int *futex_word, unsigned int expected,
		     int nr_wake, unsigned int *target_word, int nr_requeue,
		     int private)
{
  return lll_futex_syscall_ret (6, futex_word,
				__lll_private_flag (FUTEX_CMP_REQUEUE,
						    private),
