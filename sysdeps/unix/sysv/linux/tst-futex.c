...
#define BAD_FLAGS (~FUTEX_FLAG_SHARED)
...
waiter_tf (void *closure)
{
  while (__atomic_load_n (&ftx1, __ATOMIC_RELAXED) == 0)
    {
      int r = futex_wait (&ftx1, 0, FUTEX_FLAG_PRIVATE);
      TEST_VERIFY_EXIT (r == 0 || r == EAGAIN || r == EINTR);
    }
  return NULL;
}
...
do_test (void)
{
  ...
  TEST_COMPARE (futex_wait (&word, 4, FUTEX_FLAG_PRIVATE), EAGAIN);
  TEST_COMPARE (futex_wait (&word, 4, FUTEX_FLAG_SHARED), EAGAIN);

  /* Waking with no waiters reports zero woken threads.  */
  TEST_COMPARE (futex_wake (&word, INT_MAX, FUTEX_FLAG_PRIVATE), 0);

  /* Invalid flags.  */
  TEST_COMPARE (futex_wait (&word, 5, BAD_FLAGS), EINVAL);
  TEST_COMPARE (futex_wake (&word, 1, BAD_FLAGS), -EINVAL);
  TEST_COMPARE (futex_requeue (&word, 5, 0, &ftx2, 1, BAD_FLAGS), -EINVAL);

  /* Requeue with a value mismatch fails and moves no waiters.  */
  TEST_COMPARE (futex_requeue (&ftx1, 1, 0, &ftx2, 1, FUTEX_FLAG_PRIVATE),
		-EAGAIN);
     ...
     on FTX1.  */
  {
    ...
    int r;
    while ((r = futex_requeue (&ftx1, 0, 0, &ftx2, 1, FUTEX_FLAG_PRIVATE))
	   == 0)
      sched_yield ();
    TEST_COMPARE (r, 1);
    ...
    TEST_COMPARE (futex_wake (&ftx2, INT_MAX, FUTEX_FLAG_PRIVATE), 1);
    /* In case the waiter was woken spuriously and blocked on FTX1 again.  */
    futex_wake (&ftx1, INT_MAX, FUTEX_FLAG_PRIVATE);

    xpthread_join (thr);
  }

  /* The same wait/requeue/wake round-trip with FUTEX_FLAG_SHARED across
     processes, with the futex words in a shared mapping.  */
  {
	...
	while (__atomic_load_n (&sftx[0], __ATOMIC_RELAXED) == 0)
	  {
	    int r = futex_wait (&sftx[0], 0, FUTEX_FLAG_SHARED);
    ...
    int r;
    while ((r = futex_requeue (&sftx[0], 0, 0, &sftx[1], 1,
			       FUTEX_FLAG_SHARED)) == 0)
      sched_yield ();
    TEST_COMPARE (r, 1);

    __atomic_store_n (&sftx[0], 1, __ATOMIC_RELAXED);
    TEST_COMPARE (futex_wake (&sftx[1], INT_MAX, FUTEX_FLAG_SHARED), 1);
    /* In case the child was woken spuriously and blocked again.  */
    futex_wake (&sftx[0], INT_MAX, FUTEX_FLAG_SHARED);
...
/* futex_timedwait is only declared when time_t is a 64-bit type.  */
#ifdef __USE_TIME_BITS64
  {
    ...
    TEST_COMPARE (futex_timedwait (&tword, 0, CLOCK_PROCESS_CPUTIME_ID, &ts,
				   FUTEX_FLAG_PRIVATE), EINVAL);

    /* Timeouts in the past, including negative tv_sec.  */
    TEST_COMPARE (futex_timedwait (&tword, 0, CLOCK_MONOTONIC, &ts,
				   FUTEX_FLAG_PRIVATE), ETIMEDOUT);
