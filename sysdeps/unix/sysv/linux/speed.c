 ...
 * with the corresponding integers. This transformation is
 * its own inverse.
 *
...
static speed_t
__hack_legacy_speeds (speed_t speed)
{
  switch (speed)
    {
#if defined(B0) && (B0 != 0)
...
/*
 * Convert *kernel* speed_t to and from *user* speed_t, after validity
 * range check.
 */
static inline speed_t
speed_user_from_kernel (speed_t speed)
{
  return __hack_legacy_speeds (speed);
}

static inline speed_t
speed_kernel_from_user (speed_t speed)
{
  return __hack_legacy_speeds (speed);
}

/*
 * Public user space conversion functions between (user) speed_t and baud_t
 */
speed_t
cfencspeed (baud_t baud)
{
  if (invalid_baud (baud)) {
    __set_errno (EINVAL);
    return (speed_t) -1;
  }
  return __hack_legacy_speeds (baud);
}
libc_hidden_def (cfencspeed)

baud_t
cfdecspeed (speed_t speed)
{
...
cfgetospeed (const struct termios *termios_p)
{
  ...
  if (ospeed != BOTHER)
    return ospeed;
  return speed_user_from_kernel (termios_p->c_ospeed);
}
...
cfgetispeed (const struct termios *termios_p)
{
  ...
  if (ispeed != BOTHER)
    return ispeed;
  return speed_user_from_kernel (termios_p->c_ispeed);
}
libc_hidden_def (cfgetispeed)
