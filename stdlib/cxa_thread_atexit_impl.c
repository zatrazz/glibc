...
/* CONCURRENCY NOTES:
   ...
   __cxa_thread_atexit_impl does not take dl_load_lock (it used to; taking
   the lock deadlocks if this function is reached from a thread spawned by
   an ELF constructor running under dl_load_lock inside dlopen, bug 15686).
   It locates the caller's link map with _dl_find_object, which is
   async-signal-safe and does not take any loader lock, and then increments
   l_tls_dtor_count atomically.
...
__cxa_thread_atexit_impl (dtor_func func, void *obj, void *dso_symbol)
{
  ...
  /* Locate the link map of the caller without taking dl_load_lock, so that
     this function can be called from a thread spawned by an ELF constructor
     running under dl_load_lock inside dlopen (bug 15686).  _dl_find_object
     is lock-free, and the newly loaded object is added to its lookup
     structure before its constructors run.  See CONCURRENCY NOTES for why
     a racing dlclose need not be considered here.  */
  struct link_map *map;
  struct dl_find_object dfo;
  if (GLRO (dl_find_object) (dso_symbol, &dfo) == 0 && dfo.dlfo_link_map != NULL)
