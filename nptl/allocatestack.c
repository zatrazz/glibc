...
/*
  ...
  (TLS_TCB_AT_TP or TLS_DTV_AT_TP), if the kernel supports lightweight guard
...
/* For TLS_TCB_AT_TP the TLS guard sits between the stack and the static TLS
   area.  Its end is page-aligned at or below the start of static TLS.
   ...
   Returns a pointer to the page-aligned end of the TLS guard (the first byte
...
static __always_inline char *
tls_guard_start_position (const struct pthread *pd,
			  size_t tls_static_size_for_stack,
			  size_t guardsize,
			  size_t pagesize_m1)
{
		  ...
		  size_t pagesize_m1)
{
  ...
  char *tls_guard = tls_guard_start_position (pd, tls_static_size_for_stack,
...
#if ARCH_HAS_TLS_GUARD
  if (pd->guardsize > 0)
    stacktop = tls_guard_start_position (pd, tls_static_size_for_stack,
